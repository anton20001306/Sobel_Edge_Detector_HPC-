/******************************************************************************
* FILE: omp_sobel.c
* DESCRIPTION:
*   OpenMP Sobel Edge Detection with Gaussian Blur
*   HPC-style parallel implementation similar to LLNL OpenMP tutorial
* AUTHOR: Anton Praveen
* LAST REVISED: 03/12/26
******************************************************************************/

#include <omp.h>
#include <stdio.h>
#include <stdlib.h>
#include <math.h>

int sobel(int gx, int gy)
{
    int val = (int)sqrt(gx*gx + gy*gy);
    if(val > 255) val = 255;
    if(val < 0) val = 0;
    return val;
}

int main(int argc, char *argv[])
{
    int tid, nthreads;
    int width, height, maxval;
    int i,j,x,y;
    int chunk = 20;

    FILE *fp = fopen("input.pgm","rb");

    if(fp == NULL)
    {
        printf("Cannot open image\n");
        return 1;
    }

    char format[3];

    fscanf(fp,"%s",format);
    fscanf(fp,"%d %d",&width,&height);
    fscanf(fp,"%d",&maxval);
    fgetc(fp);

    unsigned char *image = malloc(width*height);
    unsigned char *blur  = malloc(width*height);
    unsigned char *edge  = malloc(width*height);

    fread(image,1,width*height,fp);
    fclose(fp);

    int G[3][3] =
    {
        {1,2,1},
        {2,4,2},
        {1,2,1}
    };

    int Gx[3][3] =
    {
        {-1,0,1},
        {-2,0,2},
        {-1,0,1}
    };

    int Gy[3][3] =
    {
        {-1,-2,-1},
        {0,0,0},
        {1,2,1}
    };

    double start = omp_get_wtime();

#pragma omp parallel shared(image,blur,edge,nthreads,chunk,width,height,G,Gx,Gy) \
                     private(tid,i,j,x,y)
{
    tid = omp_get_thread_num();

    if(tid == 0)
    {
        nthreads = omp_get_num_threads();
        printf("Starting Sobel Edge Detection using %d threads\n",nthreads);
    }

    /******** Gaussian Blur ********/

#pragma omp for schedule(static,chunk)
    for(i=1;i<height-1;i++)
    {
        for(j=1;j<width-1;j++)
        {
            int sum = 0;

            for(x=-1;x<=1;x++)
            {
                for(y=-1;y<=1;y++)
                {
                    sum += image[(i+x)*width+(j+y)] * G[x+1][y+1];
                }
            }

            blur[i*width+j] = sum/16;
        }
    }

    /******** Sobel Edge Detection ********/

#pragma omp for schedule(static,chunk)
    for(i=1;i<height-1;i++)
    {
        printf("Thread %d processing row %d\n",tid,i);

        for(j=1;j<width-1;j++)
        {
            int gx = 0;
            int gy = 0;

            for(x=-1;x<=1;x++)
            {
                for(y=-1;y<=1;y++)
                {
                    int pixel = blur[(i+x)*width+(j+y)];

                    gx += pixel * Gx[x+1][y+1];
                    gy += pixel * Gy[x+1][y+1];
                }
            }

            edge[i*width+j] = sobel(gx,gy);
        }
    }

} /* end parallel region */

    double end = omp_get_wtime();

    printf("Execution Time: %f seconds\n",end-start);

    FILE *out = fopen("output_parallel.pgm","wb");

    fprintf(out,"P5\n%d %d\n255\n",width,height);

    fwrite(edge,1,width*height,out);

    fclose(out);

    free(image);
    free(blur);
    free(edge);

    printf("Parallel edge detection finished\n");

    return 0;
}